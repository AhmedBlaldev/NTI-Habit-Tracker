  ...
  try {
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message: "daysOfWeek is required for custom frequency",
        });
    }
    ...
    if (Array.isArray(daysOfWeek)) {
      ...
      if (invalidDay !== undefined) {
        return res
          ...
          .json({
            success: false,
            message: "daysOfWeek must be numbers between 0 and 6",
          });
      }
    }
    ...
    if (startDate && endDate) {
      ...
      if (isNaN(s) || isNaN(e) || s > e) {
        return res
          .status(400)
          .json({ success: false, message: "Invalid startDate/endDate range" });
      }
    }
    ...
    if (existing) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "You already have a habit with this title",
        });
    }
    ...
    logger.error("Create habit failed", {
      error: error.message,
      stack: error.stack,
      userId: req.user?.id,
    });
...
const updateHabit = async (req, res) => {
  try {
    ...
    logger.error("Update habit failed", {
      error: error.message,
      stack: error.stack,
      userId: req.user?.id,
    });
...
const deleteHabit = async (req, res) => {
  try {
    ...
    logger.error("Delete habit failed", {
      error: error.message,
      stack: error.stack,
      userId: req.user?.id,
    });
